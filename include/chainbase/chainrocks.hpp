...
#define CHAINBASE_DEFAULT_CONSTRUCTOR( OBJECT_TYPE )            \
    ...
    public:
        ...
        const rocksdb::TransactionDBOptions& transactiondb_options() {
            return _transactiondb_options;
        }

        const rocksdb::TransactionOptions& transaction_options() {
            return _transaction_options;
        }
    ...
    private:
        rocksdb::Options              _general_options;
        rocksdb::TransactionDBOptions _transactiondb_options;
        rocksdb::TransactionOptions   _transaction_options;
        rocksdb::ReadOptions          _read_options;
        rocksdb::WriteOptions         _write_options;
    };
    ...
    public:
        ...
        {
            _status = rocksdb::TransactionDB::Open(_options.general_options(),
                                                   _options.transactiondb_options(),
                                                   _data_dir.string().c_str(),
                                                   &_transaction_database_ptr);
            _check_status();
        }
        ...
        ~rocksdb_database() {
            _transaction_database_ptr->Close();
            _check_status();
        
            if (_transaction_occurring()) {
                _end_transaction_session();
            }
        
            delete _transaction_database_ptr;
        }

        void put(const std::string& key, const std::string& value) {
            if (!_transaction_occurring()) {
                _begin_transaction_session();
            }
            _status = _transaction_ptr->Put(key, value);
            _check_status();
        }
        ...
        void remove(const std::string& key) {
            ...
            _status = _transaction_ptr->Delete(key);
            _check_status();
        }
        ...
        void get(const std::string& key, std::string &value) {
            ...
            _status = _transaction_database_ptr->Get(_options.read_options(), key, &value);
            _check_status();
        }
   
        // // Deteremined by user-defined merge operator
        // void merge(const chainrocks_key& key, const chainrocks_value& value) {
        //    _status = _database->Merge(_options.write_options(), to_slice_type(key), to_slice_type(value));
        //    _check_status();
        // }

        void commit() {
            _status = _transaction_ptr->Commit();
            _end_transaction_session();
            _check_status();
        }
         
    private:
        rocksdb::TransactionDB*                 _transaction_database_ptr;
        rocksdb::Transaction*                   _transaction_ptr;
        rocksdb::Status                         _status;
        bfs::path                               _data_dir;
        // std::vector<abstract_index*>            _index_list;
        // std::vector<unique_ptr<abstract_index>> _index_map;
        rocksdb_options                         _options;
        bool                                    _transaction_session{false};

        // template<typename T>
        // inline const rocksdb::Slice to_slice_type(const T& t) const {
        //     return static_cast<const rocksdb::Slice>(t);
        // }
        ...
        inline bool _transaction_occurring() const {
            return _transaction_session;
        }

        inline void _begin_transaction_session() {
            _transaction_session = true;
            _transaction_ptr = _transaction_database_ptr->BeginTransaction(_options.write_options());
        }

        inline void _end_transaction_session() {
            _transaction_session = false;
            delete _transaction_ptr;
        }
    };
    ...
    public:
        ...
        session start_undo_session( bool enabled ) {
            if( enabled ) {
                ...
                _stack.back().old_next_id = _next_id;
                _stack.back().revision = ++_revision;
                // _db.emplace_back( _indices.get_allocator() );
                // _db.back().old_next_id = _next_id;
                // _db.back().revision = ++_revision;
                return session( *this, _revision );
            } else {
                return session( *this, -1 );
            }
    ...
    private:
        ...
        rocksdb_database                                                          _db{"/tmp/chainrocks/test"};
